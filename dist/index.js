...
(() => {
  ...
  var prizesAnimation = () => {
    ...
    const grandTotalElement = document.querySelector(SELECTORS8.grandTotal);
    const podium1Inner = document.querySelector(`${SELECTORS8.podium1} ${SELECTORS8.podiumInner}`);
    const podium2Inner = document.querySelector(`${SELECTORS8.podium2} ${SELECTORS8.podiumInner}`);
    const podium3Inner = document.querySelector(`${SELECTORS8.podium3} ${SELECTORS8.podiumInner}`);
    const podium1Amount = document.querySelector(`${SELECTORS8.podium1} ${SELECTORS8.podiumAmount}`);
    const podium2Amount = document.querySelector(`${SELECTORS8.podium2} ${SELECTORS8.podiumAmount}`);
    const podium3Amount = document.querySelector(`${SELECTORS8.podium3} ${SELECTORS8.podiumAmount}`);
    const podiumContainer = document.querySelector(SELECTORS8.podiumContainer);
  ...
  var introAnimation = () => {
    ...
    const headingWrap = document.querySelector(SELECTORS8.headingWrap);
    const heading = document.querySelector(SELECTORS8.heading);
  ...
  var animateNavLogo = (debug = false) => {
    ...
    const logo = document.querySelector(SELECTORS8.logo);
    if (!logo) {
  ...
  var orderWinnerCMSList = () => {
    ...
    const cmsList = document.querySelector(SELECTORS8.CMS_LIST);
    if (!cmsList) {
    ...
    const sortedItems = items.sort((a, b) => {
      const categoryA = a.getAttribute(SELECTORS8.CMS_CATEGORY) || "";
      const categoryB = b.getAttribute(SELECTORS8.CMS_CATEGORY) || "";
      const positionA = parseInt(a.getAttribute(SELECTORS8.CMS_POSITION) || "0", 10);
      const positionB = parseInt(b.getAttribute(SELECTORS8.CMS_POSITION) || "0", 10);
  ...
  var filters = () => {
    ...
    try {
      const filterGroups = document.querySelectorAll(SELECTORS5.FILTER_GROUP);
  ...
  function initializeFilterSystem(groupId) {
    try {
      ...
      const updateTriggerStyle = (trigger, isActive) => {
        try {
          if (isActive) {
      ...
      const updateAllTriggerStyles = () => {
        try {
          const triggers = filterGroup.querySelectorAll(SELECTORS5.FILTER_TRIGGER);
      ...
      const setInitialFilter = () => {
        const firstTrigger = filterGroup.querySelector(SELECTORS5.FILTER_TRIGGER);
        if (!firstTrigger) {
  ...
  var heroLogoMarquee = () => {
    const initMarquee = () => {
      try {
        ...
        const marqueeRows = document.querySelectorAll(SELECTORS8.marqueeRow);
        const marqueeRowInners = document.querySelectorAll(SELECTORS8.marqueeRowInner);
        if (!isTablet()) {
          marqueeRows.forEach((marqueeRow) => {
            ...
            gsapWithCSS.killTweensOf(`${SELECTORS8.marqueeRow} > *`);
            try {
  ...
  var inspirationImageRowPadding = () => {
    const imageRowList = document.querySelector(SELECTORS6.imageRowList);
    const header = document.querySelector(SELECTORS6.header);
  ...
  var inspirationImageSlider = () => {
    const imageSlider = document.querySelector(SELECTORS6.imageRow);
    const nextButton = document.querySelector(SELECTORS6.nextButton);
    const prevButton = document.querySelector(SELECTORS6.prevButton);
    const header = document.querySelector(SELECTORS6.header);
  ...
  var parallaxBackground = () => {
    ...
    const parallaxBg = document.querySelectorAll(SELECTORS8.BG_ELEMENT);
    if (parallaxBg.length === 0) {
      console.error(`No parallax bg elements found, set using: ${SELECTORS8.BG_ELEMENT}`);
      return;
    }
  ...
  var flipWinnerItemPosition = () => {
    const cmsList = document.querySelector(SELECTORS7.cmsList);
    if (!cmsList) {
      console.error("CMS list not found");
      return;
    }
    Array.from(cmsList.children).forEach((item) => {
      const position = item.getAttribute(SELECTORS7.itemPosition);
      const infoWrapper = item.querySelector(SELECTORS7.infoWrapper);
      const itemLayout = item.querySelector(SELECTORS7.itemLayout);
      if (position === "2") {
