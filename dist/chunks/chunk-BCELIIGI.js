...
var updateAllTriggerStyles = (filterGroup, activeFilter, source, groupId) => {
  ...
  const triggers = filterGroup.querySelectorAll(SELECTORS2.FILTER_TRIGGER);
...
function initializeFilterSystem(groupId) {
  ...
  try {
    ...
    const setInitialFilter = () => {
      const firstTrigger = filterGroup.querySelector(SELECTORS2.FILTER_TRIGGER);
      if (!firstTrigger) {
...
var filters = () => {
  sortMobileWinnerSwiperItemPosition();
  try {
    const filterGroups = document.querySelectorAll(SELECTORS2.FILTER_GROUP);
...
function getCurrentFilterValue(groupId) {
...
var getSwiperInstance = (selector) => {
